 ...
 * Ne fonctionne que côté client (pas en SSR)
 */
async function loadSupabaseConfig(): Promise<void> {
  ...
  try {
    // Utiliser le proxy Next.js (fonctionne seulement côté client)
    // Le proxy dans next.config.js redirige /api/* vers http://localhost:8000/api/*
...
// Charger la configuration seulement côté client
if (typeof window !== 'undefined') {
...
export const resetPassword = async (email: string): Promise<{ error: Error | null }> => {
  ...
  const redirectUrl = typeof window !== 'undefined' ? `${window.location.origin}/auth/reset-password` : '/auth/reset-password'
